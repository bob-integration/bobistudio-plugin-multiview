...
OUT_FRAME_SIZE = OUT_WIDTH * OUT_HEIGHT * 3 // 2
HEADER_SIZE    = 64
RING_SIZE      = 10
...
def rgba_to_yuv(img):
    ...
    u2 = ((u[0::2, 0::2].astype(np.uint16) + u[0::2, 1::2]
         + u[1::2, 0::2] + u[1::2, 1::2]) // 4).astype(np.uint8)
    v2 = ((v[0::2, 0::2].astype(np.uint16) + v[0::2, 1::2]
         + v[1::2, 0::2] + v[1::2, 1::2]) // 4).astype(np.uint8)
    a2 = np.maximum(np.maximum(a[0::2, 0::2], a[0::2, 1::2]),
...
# orchestrateur, qui réécrit in_w/in_h). La géométrie reste issue de FLUX_CONFIG.
...
while True:
    ...
    canvas_u = np.full((OUT_HEIGHT//2, OUT_WIDTH//2), 128, dtype=np.uint8)
    canvas_v = np.full((OUT_HEIGHT//2, OUT_WIDTH//2), 128, dtype=np.uint8)
    ...
    for i in range(len(FLUX_CONFIG)):
        ...
        try:
            ...
            src_u = np.frombuffer(bytes(shm_view[offset + in_w*in_h:offset + in_w*in_h + in_w*in_h//4]),
                                  dtype=np.uint8).reshape(in_h//2, in_w//2)
            src_v = np.frombuffer(bytes(shm_view[offset + in_w*in_h + in_w*in_h//4:offset + frame_size]),
                                  dtype=np.uint8).reshape(in_h//2, in_w//2)

            dst_y = resize_plane(src_y, vh,    video_w)
            dst_u = resize_plane(src_u, vh//2, video_w//2)
            dst_v = resize_plane(src_v, vh//2, video_w//2)

            canvas_y[y:y+vh,                 video_x:video_x+video_w]            = dst_y
            canvas_u[y//2:y//2+vh//2,        video_x//2:video_x//2+video_w//2]   = dst_u
            canvas_v[y//2:y//2+vh//2,        video_x//2:video_x//2+video_w//2]   = dst_v
        except Exception:
            canvas_y[y:y+vh, video_x:video_x+video_w] = 0
